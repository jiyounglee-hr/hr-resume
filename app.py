...
- 이력서와 채용공고의 연결고리를 고려해 질문을 구성하세요."""},
    ...
    st.markdown("###### 📒 면접 순서")
    st.markdown("""
    1. 면접관 사전 미팅: 면접 시작 10분 전, 면접관 간 진행 방식 및 역할 분담 등을 간단히 조율합니다.

    2. 면접 시작 및 오프닝:
        ① 지원자 입장 후, 가볍게 인사하며 면접 본 질문으로 들어가기 전 편안한 분위기를 유도합니다. (예시: "식사는 하셨나요?", "오시는 데 불편하진 않으셨나요?", "많이 긴장 되시죠? 편안하게 생각하세요!")
        ② 면접관의 소속과 직책을 소개하고, 채용 직무 및 면접 방식에 대해 간단히 설명해주세요.

    3. 자기소개 요청: 지원자에게 경력 중심의 자기소개를 요청합니다. 자기소개 중에는 가능한 한 eye-contact을 유지하며 부드러운 표정으로 경청해 주세요.

    4. 자기소개 기반 질문: 자기소개 내용 중 궁금하거나 더 구체적인 내용을 중심으로 추가 질문을 진행합니다.

    5. 직무역량 및 적성 관련 질문: 지원서류를 참고하여, 직무 적합성과 역량을 확인할 수 있는 질문을 진행합니다. (✅ 위에 '🤖AI가 면접질문을 뽑아 드려요.' 기능 참고)

    6. 입사 관련 사항 확인: 지원자의 입사 가능일 등 일정 관련 정보를 확인해 주세요.(※ 연봉 확인은 2차 면접에서 진행됩니다.)

    7. 면접 종료 및 안내
        ① 지원자에게 궁금한 점이 있는지 확인하고, 다음 전형 일정을 간단히 안내합니다. (예시: "면접 결과는 인사팀에서 개별 안내드릴 예정입니다. 합격 시 2차 면접은 별도 일정으로 조율됩니다.)
        ② 마지막에는 따뜻한 격려의 인사를 전해 주세요.(예시: "면접 보시느라 고생 많으셨습니다. 좋은 결과 있길 바랍니다. 수고하셨습니다.")

    8. 면접 평가서 제출: 면접 종료 후, ‘📝면접평가서 제출 페이지’에서 면접자별 평가서를 작성하고 제출해 주세요. (✅제출된 평가는 자동 저장되며, 제출 완료 후 PDF 다운로드도 가능합니다.)
