...
if st.session_state['current_page'] == "resume":
    ...
    st.markdown("""
        ① 서류접수 및 전달 : 서류접수 및 전달 : 인사팀에서 접수된 지원서를 확인하고, 기본 사항(지원 직무, 경력 등)을 검토한 후 해당 면접관에게 검토 요청을 드립니다.
        <small style='color: #666666;'>
            (아래 '🤖 AI가 이력서 분석을 도와드려요!'를 활용해 보세요)
        </small>
        ② 서류검토 및 회신 : 면접관께서는 서류 검토 결과를 채용 채팅(팀즈)을 통해 인사팀에 회신해주세요.

        ③ 면접일정 확인 : 면접 일정 확인 및 통보: 합격자에 한해 인사팀이 면접관 및 지원자 일정 확인 후 1차 면접 일정을 조율하며, 불합격자는 인사팀에서 지원자에게 이메일로 개별 통보합니다.
        """, unsafe_allow_html=True)
